...
def save_to_excel(devices: List[Device], output_file: str):
    ...
    software_headers = [
        "Устройство", "Software Name", "Версия", "Вендор"
    ]
    ...
    vuln_headers = [
        "Устройство", "Kaspersky ID", "Product Name", "Severity", 
...
def main():
    try:
        ...
        # Обработка нестандартного формата (без обрамляющих [])
        if not content:
            data = []
        elif content.startswith('[') and content.endswith(']'):
            data = json.loads(content)
        else:
            # Удаление завершающей запятой если есть
            if content.endswith(','):
